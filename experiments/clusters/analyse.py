...
def analyse(interpreter):
    ...
    print("{:>10} {:>8} {:>8} {:>8} {:>8} {:>8}".format(
        "Risk level", "Clusters", "Average", "Min", "Max", "SD"
    ))
    ...
    for risk_level, indices in zip(['INFO', 'LOW', 'MEDIUM', 'HIGH', 'ATTACK', 'SUSPICIOUS'], indices_levels):
        ...
        print("{:10} {:8} {:8.1f} {:8} {:8} {:8.1f}".format(
            risk_level, unique.shape[0], counts.mean(), counts.min(),
    ...
    # Print total
    print("{:10} {:8} {:8.1f} {:8} {:8} {:8.1f}".format(
        'Total', unique.shape[0], counts.mean(), counts.min(),
        counts.max(), counts.std(),
    ))
